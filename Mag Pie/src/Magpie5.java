...
public class Magpie5 {
	...
	public String getResponse(String statement) {
		...
		// Responses which require transformations
			...
			response = transformILikeArray(statement);

		} else if (findKeyword(statement, "I dislike", 0) >= 0) {
			response = transformIDislikeArray(statement);
	...
	private String transformILikeArray(String statement) {
		statement = statement.trim();
		String lastChar = statement.substring(statement.length() - 1);
		if (lastChar.equals(".")) {
			statement = statement.substring(0, statement.length() - 1);
		...
		String restOfStatement = statement.substring("I like".length()).trim();
		like[psnOfILike] = statement;
		return "You like " + restOfStatement;
		

	}

	private String transformIDislikeArray(String statement) {
		statement = statement.trim();
		String lastChar = statement.substring(statement.length() - 1);
		if (lastChar.equals(".")) {
			statement = statement.substring(0, statement.length() - 1);
		...
		int psnOfIDisike = findKeyword(statement, "I dislike", 0);
		String restOfStatement = statement.substring("I dislike".length()).trim();
		return "You dislike " + restOfStatement;
	}
